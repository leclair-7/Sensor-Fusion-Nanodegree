...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    //renderPointCloud(viewer,cloud,"ego");
    ProcessPointClouds<pcl::PointXYZ> * pointProcessor = new ProcessPointClouds<pcl::PointXYZ>();

    int iterations = 100;
    float distThresh = .2;

    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr> res = pointProcessor->SegmentPlane(cloud, iterations,distThresh);
    //renderPointCloud(viewer,res.first,"obstCloud",Color(1,0,0));
    //renderPointCloud(viewer,res.second,"planeCloud",Color(0,1,0));
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloudClusters = pointProcessor->Clustering(res.first, 1.0, 3, 30);
